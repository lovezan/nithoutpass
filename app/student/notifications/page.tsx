...
export default function StudentNotificationsPage() {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      if (parsedData.isNewStudent === true) {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      setIsLoading(true)
      try {
        // Fetch notifications for this student (both parent and student types)
        const response = await fetch(`/api/notifications?recipientId=${student.id}&types=parent,student`)
        if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-10">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-muted-foreground">View all notifications for your outpasses</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Recent notifications about your outpasses</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
              ...
              {notifications.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="mt-2 md:mt-0 md:ml-4 flex items-center">
                      <Button variant="outline" size="sm" asChild>
                        <Link href={`/student/outpass/${notification.outpassId}`}>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">
                    You'll receive notifications here when there are updates to your outpasses
                  </p>
                </div>
              )}
            </div>
